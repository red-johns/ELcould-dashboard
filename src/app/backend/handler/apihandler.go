...
import (
	...
	"github.com/kubernetes/dashboard/src/app/backend/resource/navgroup"
...
func CreateHTTPAPIHandler(iManager integration.IntegrationManager, cManager clientapi.ClientManager,
	...
	apiV1Ws.Route(
		apiV1Ws.GET("/navGroup").
			To(apiHandler.handleGetNavGroup).
			Writes(navgroup.NavGroup{}))

	return wsContainer, nil
}

// TODO: 获取导航菜单
func (apiHandler *APIHandler) handleGetNavGroup(request *restful.Request, response *restful.Response) {
	navGroup, err := navgroup.GetNavGroup()
	if err != nil {
		kdErrors.HandleInternalError(response, err)
		return
	}
	response.WriteHeaderAndEntity(http.StatusOK, navGroup)
}
